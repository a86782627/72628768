# Create a new PowerShell process that runs hidden
$scriptBlock = {
    # Download the image
...
# Run the script block in a hidden window
Start-Process powershell.exe -ArgumentList "-NoProfile -NonInteractive -WindowStyle Hidden -Command & {$scriptBlock}"